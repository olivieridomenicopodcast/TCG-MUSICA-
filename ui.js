  ...
  playersToShow.forEach(pi => {
    ...
    p.hand.forEach(cardId => {
      ...
      div.innerHTML = `
        ...
        ${card.type === "musician" ? handMusicianStatsHtml(card) : ""}
...
function handMusicianStatsHtml(card) {
  return `
    <div class="hc-stats">
      <div class="hc-stats-row"><span></span><span class="hc-stat hc-N">${card.N}</span><span></span></div>
      <div class="hc-stats-row"><span class="hc-stat hc-O">${card.O}</span><span class="hc-stat hc-central">${card.central}</span><span class="hc-stat hc-E">${card.E}</span></div>
      <div class="hc-stats-row"><span></span><span class="hc-stat hc-S">${card.S}</span><span></span></div>
    </div>
  `;
}
